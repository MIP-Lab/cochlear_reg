...
parser.add_argument("--nii_folder", help="path to test nifti files")
...
class MyTrainer(Trainer):
    ...
    def test_loss(self, model, input_data):
        ...
        pat_activation = model.dp1_conv1(torch.cat([input_data['vol'], x0], dim=1))

        combined_activation = pat_activation + input_data['atlas'] - model.dp1_conv1.bias[None, :, None, None, None]
        ddf = model(combined_activation)
...
inshape = [128, 128, 128]  
model = LocalNet(inshape[0], use_activation_map_as_input=True)

test_ds = CochlearCTCrop(nii_path=args.nii_folder, cases=['PID_1689_PLANID_1096_LEFT'], 
                         vtx_path=None, use_atlas_activation=True, aug=False)
